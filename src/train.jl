	...
	# function train!(model::LNMMSB; iter::Int64=150, etol::Float64=1, niter::Int64=1000, ntol::Float64=1.0/(model.K^2), viter::Int64=10, vtol::Float64=1.0/(model.K^2), elboevery::Int64=10, mb::MiniBatch,lr::Float64)
		...
		model.μ_var=deepcopy(_init_μ)
		# xx=((2*model.N)/model.K)
		# temp_μ = similar(_init_μ)
		# temp_μ .=deepcopy(log.(xx/(2*model.N - xx)))
		# model.μ_var = deepcopy(_init_μ)
		...
		for i in 1:iter
			...
			for d in shuffled
				if isalink(model, "network", d.src, d.dst)
					l = Link(d.src, d.dst, _init_ϕ,_init_ϕ)
					if l in mb.mblinks
						continue;
					else
						push!(mb.mblinks, l)
			...
			# end
			# model.μ_var[model.mbids,:]=deepcopy(temp_μ[model.mbids,:])##I added instead of the above)
			...
			for a in mb.mbnodes
				if !isfullsample
					...
					# count_Λ[a]+=1
					count_a[a] += 1
					expectedvisits = (iter/(model.N/model.mbsize))
					...
					# end
					# Plots.plot(1:expectedvisits,
					# [(expectedvisits/(expectedvisits+Float64(i-1.0)))^(.5) for i in 1:expectedvisits])
			...
			if checkelbo || i == 1
		        ...
		        for pair in collect(keys(model.ho_map))

		            edge_lik = edge_likelihood(model,pair, model.est_β)
				...
				link_ratio = nnz(model.network)/(model.N*(model.N-1))
		...
		p3=Plots.heatmap(true_θs, yflip=true, color=:Greys)
		PyPlot.savefig("Docs/Writeups/Proj1/true1000.png")
		# p4=Plots.heatmap(est[:,[7,2,1,6,5,3,4]], yflip=true, color=:Greys)
		p4=Plots.heatmap(est[:,[18,23,15,9,4,11,19,2,27,22,5,7,29,21,24,20,17,12,14,25,16,28,13,8,10]], yflip=true, color=:Greys)
		PyPlot.savefig("Docs/Writeups/Proj1/est1000.png")
