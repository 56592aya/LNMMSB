...
export EPSILON, Dyad, VectorList, MatrixList, Matrix2d, Matrix3d, Network

export Link, NonLink, MiniBatch, Map
export digamma, digamma_, lgamma, lgamma_, logsumexp, softmax, sort_by_argmax!
...
mutable struct Training
  ...
  trainingnot::Dict{Int64,Vector{Int64}}
  function Training()
    traininglinks=Vector{Link}()
    trainingnonlinks=Vector{NonLink}()
    trainingallnodes=Set{Int64}()
    trainingnot=Dict{Int64,Vector{Int64}}()
    new(traininglinks,trainingnonlinks,trainingallnodes,trainingnot)
  end
end
